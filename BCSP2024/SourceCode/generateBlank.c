...
// This function creates randomly a new problem.
void new_problem() {
    int i, j, r;
    for (i = 0; i < M; i++)
        for (j = 0; j < K; j++) {
...
// This function writes the puzzle to a file
// This function writes an empty file
void write_to_file(int id, char *filename) {
    FILE *fout;
    char s[255];
    sprintf(s, "%s_%d.txt", filename, id);
    fout = fopen(s, "w");
    if (fout == NULL) {
...
// Generate problems from id1 to id2
void generate_problems(int id1, int id2, char *filename) {
...
int main(int argc, char **argv) {
    ...
    generate_problems(id1, id2, argv[1]);
