...
void display_elapsed_time(double elapsed_time);
void walkSat(char **argv);
int dpll(char **argv);
int dpll_recursive(int *vector, int depth, long *steps);
...
int dpll(char **argv) {
    ...
    while  (((double)(clock() - t1)) / CLOCKS_PER_SEC < TIMEOUT) {  // Όσο δεν έχουν περάσει 10 δευτερόλεπτα
        if (dpll_recursive(vector, 0, &steps)) {
            t2 = clock();
...
int dpll_recursive(int *vector, int depth, long *steps) {
    ...
    vector[depth] = 1;
    (*steps)++;
    if (valid(vector) && dpll_recursive(vector, depth + 1, steps)) {
        return 1;
    }
...
void walkSat(char **argv) {
    ...
    long restarts=0, steps=0;
    int best_change;
    double temperature = 1000.0; // Αρχική θερμοκρασία
    double cooling_rate = 0.95; // Ρυθμός ψύξης
    vector=(int*) malloc(N*sizeof(int));
    ...
    while (h>0 && temperature > 0.01) { // Ελέγχουμε και τη θερμοκρασία
        // Check for time limit
        t=clock();
        if (t-t1>CLOCKS_PER_SEC*TIMEOUT) {
            t2=clock();
        ...
        for(i=0;i<N;i++) {      // For each proposition...
            vector[i]=-vector[i];   // ...if we flip its value...
            h1=count(vector);   // ...how many sentences are not satisfied?
            if (h1<h2 || exp((h2 - h1) / temperature) > (double)rand() / RAND_MAX) { // Εδώ εφαρμόζουμε την αλλαγή με πιθανότητα exp((h2 - h1) / temperature)
                h2=h1;          // ...remember it!
                best_change=i;
            }
            vector[i]=-vector[i];   // Cancel the last flip of value.
...
void syntax_error(char **argv) {
	...
	printf("where:\n");
	printf("<method> is either 'hill' or 'depth' or 'walksat' or 'dpll' (without the quotes)\n");
